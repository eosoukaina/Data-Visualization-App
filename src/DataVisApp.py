...
class Visualization:
    def __init__(self, window):
        ...
        exit_image = Image.open("/resources/exit.png")
...
def explore():
    ...
    heatmap_path = "/resources/Heatmap.png"
    ...
    pairplot_path = "/resources/pairplot.png"
...
def home():
    ...
    image = image.resize((window_width, window_height))  # Ensure it fits the window
    global background_photo
    background_photo = ImageTk.PhotoImage(image)

    # Center the background image
    canvas.create_image(window_width/2, window_height/2, anchor=tk.CENTER, image=background_photo)

    # Position the button manually just under the "Data Visualization App" text in the image
    # From your uploaded image, that section is around x=180, y=290 (adjust if needed)
    button_start = tk.Button(canvas, text="START", command=start, height=2, width=12,
                             bg="#3d66eb",fg="white",highlightbackground="white", font=("Helvetica", 14, "bold"))

    # Adjust x and y to match your image's text placement
    canvas.create_window(225, 600, anchor=tk.NW, window=button_start)
